"""Explanation-quality scoring with optional LLM enrichment."""
...
class ExplanationQualityEvaluator:
    """Evaluate explanation quality with deterministic fallback."""

    # In-process cache keeps repeated scenario executions reproducible.
    ...
    def evaluate(
        ...
        base_explanation_quality: float,
        context: Dict[str, float],
    ) -> Dict[str, Any]:
        """Return final score and diagnostics for one founder/VC pair."""
        self.requests += 1
        base_score = _clamp(base_explanation_quality)
        ...
        if cached is not None:
            ...
            final = _clamp((1.0 - self.llm_blend_weight) * base_score + self.llm_blend_weight * llm_score)
            return {
                "score": final,
                "source": "llm_blended_cached",
                "llm_score": llm_score,
                "makes_sense": bool(cached.get("makes_sense", llm_score >= 0.5)),
            }
        ...
        self._GLOBAL_SCORE_CACHE[cache_key] = {
            "llm_score": llm_score,
            "makes_sense": bool(llm_result.get("makes_sense", llm_score >= 0.5)),
        }
        final = _clamp((1.0 - self.llm_blend_weight) * base_score + self.llm_blend_weight * llm_score)
        return {
            "score": final,
    ...
    def _score_with_llm(
        ...
        if self._llm_client is None:
            return {"llm_score": None, "makes_sense": None}

        prompt = (
            "Rate whether this startup-investor match explanation is sensible.\n"
            "Return strict JSON with keys: score (0..1), makes_sense (bool), reason (max 20 words).\n"
        ...
        try:
            response = self._llm_client.generate(
                prompt=prompt,
                system=(
                    "You are a strict evaluator of startup-VC match explanations. "
            ...
            logger.warning("LLM explanation-quality scoring failed: %s", exc)
            return {"llm_score": None, "makes_sense": None}
    ...
    def _build_cache_key(
        ...
        key_payload = {
            "founder": ExplanationQualityEvaluator._compact_profile(founder_profile, founder=True),
            "vc": ExplanationQualityEvaluator._compact_profile(vc_profile, founder=False),
            "context": context,
        }
    ...
    def _parse_llm_response(response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            makes_sense = payload.get("makes_sense")
            if not isinstance(makes_sense, bool):
                makes_sense = score >= 0.5
            return {"llm_score": score, "makes_sense": makes_sense}
        except Exception:
            match = _SCORE_PATTERN.search(text)
            if match is None:
                return {"llm_score": None, "makes_sense": None}
            score = _clamp(float(match.group(1)))
            return {"llm_score": score, "makes_sense": score >= 0.5}
