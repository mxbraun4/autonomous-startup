...
- preserve the Build-Evaluate-Learn loop
- improve matching quality over iterations
...
- `docs/PRODUCT_VISION.md`: product direction, value proposition, acquisition layer
- `docs/CUSTOMER_SIMULATION.md`: constrained customer model, state machines, parameters
- `docs/next_steps.md`: current priorities and open work items

## Workstream Priorities
1. Matching quality and explainability
2. Acquisition layer (articles + utility tools) with clear funnel mapping
...
- Workspace live preview (auto-refreshes on file changes):
...
- `src/data/database.py`: SQLite persistence for startups and VCs
- `src/framework/storage/`: semantic/episodic/procedural memory components
...
- `scripts/serve_workspace.py`: standalone workspace preview with live-reload
...
- Preserve constrained customer simulation assumptions from `docs/CUSTOMER_SIMULATION.md`:
